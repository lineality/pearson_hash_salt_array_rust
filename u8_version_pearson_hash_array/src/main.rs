...
//!    pseudo-random 8×8 chess boards, sweeps N
//!    (e.g. 10,000) Fisher-Yates
...
mod u8_pearson_hash_salt_array_rust;
mod u8_pearson_hash_tools;
mod u8_table_finder;
...
const DEMO_SALTS: [u8; 4] = [0x01, 0x0F, 0xFF, 0xBE];
...
const FINDER_SALTS: [u8; 4] = [0x01, 0xF0, 0xFF, 0xEE];
...
fn demo_section_salt_array_hash() -> Result<(), Error> {
    ...
    println!("    (input bytes) followed by (salt[i] as one byte).");
    println!("No concatenation buffer is allocated; the input is hashed once");
    println!("and each salt continues the running hash state with a single");
    println!("Pearson step. Salts are u8, so there is no byte-order encoding");
    println!("and hashes are identical across all host architectures.");
    println!();
