// src/u8_pearson_hash_salt_array_rust.rs

//! # `pearson_hash_salt_array_rust` — Production Pearson Hashing Module
//!
//! u8 version
//! u128 version also available for where hashes need to be
//! non-trivial to guess or repeat
//!
//! This crate uses `u8` salts: each salt is one byte and produces
//! one byte of output. This is appropriate when a small, fast,
//! non-cryptographic hash is sufficient (hash-table bucket
//! selection, Bloom-filter multi-hashing, dispersal, etc.).
//!
//! A separate `u128`-salt variant of this crate exists for use
//! cases where larger per-salt entropy is wanted (e.g. salts that
//! should be harder to guess or replay). The two variants are
//! source-compatible at the API level; only the salt type differs.
//!
//! In cases where a basic hash is needed, u8 is fine.
//!
...
//!
//!    With `u8` salts, each "salt step" is exactly one Pearson XOR
//!    and table lookup, so the per-salt cost beyond hashing the input
//!    once is a single operation.
//!
//! ## Security and Scope
//!
...
mod tests {
    ...
    fn test_salt_array_distinct_salts_give_varied_output() {
        let salts: [u8; 4] = [0x01, 0x0F, 0xFF, 0xBF];
    ...
    fn test_salt_array_matches_concatenated_equivalent() {
        ...
        for (i, salt) in salts.iter().enumerate() {
            ...
            // Capacity = input length + 1 byte for the u8 salt.
            let mut concatenated: Vec<u8> = Vec::with_capacity(input.len() + 1);
            concatenated.extend_from_slice(input);
            concatenated.push(*salt);
